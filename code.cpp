...
private:
    ...
    {
        if (n == 0)
            return "Unknown (N not specified)";
        if (n <= 20)
            return "O(2^N) or O(N!) acceptable (Exponential)";
        if (n <= 50)
            return "O(2^(N/2)) or O(N^3) acceptable";
        if (n <= 200)
            return "O(N^3) acceptable";
        if (n <= 2000)
            return "O(N^2) acceptable";
        if (n <= 50000)
            return "O(N log N) required";
        if (n <= 200000)
            return "O(N log N) or O(N√N) required";
...
public:
    ...
    AnalysisResult analyze(const string &problem, const string &constraints)
    {
        ...
        if (n <= 20)
        {
            if (scores["BACKTRACKING"] > 0)
                scores["BACKTRACKING"] += 10;
        }

        if (n > 50000 && n <= 200000)
        {
            if (scores["DIJKSTRA"] > 0)
                scores["DIJKSTRA"] += 5;
            if (scores["SEGMENT_TREE"] > 0)
                scores["SEGMENT_TREE"] += 5;
            if (scores["KMP"] > 0)
                scores["KMP"] += 5;
        ...
        if (n > 200000)
        {
            if (scores["FLOYD_WARSHALL"] > 0)
                scores["FLOYD_WARSHALL"] -= 10;
        }
...
void runTests() {
    ...
    // If file not found → create default test cases
    if(!file) {
    ...
    while(getline(file, line)) {
        if(line.find("Problem:") != string::npos) {
            ...
            tests.push_back({problem, constraints, expected});
    ...
    if(choice == 0) {
        ...
        for(int i = 0; i < tests.size(); i++) {
            ...
            AnalysisResult result = advisor.analyze(tests[i].problem, tests[i].constraints);
            ...
            if(tests[i].expected == result.best.name) {
                cout << "Status:   PASSED\n";
            } else {
                cout << "Status:   FAILED\n";
    ...
    // -------- RUN SINGLE TEST --------
        ...
        cout << "\n===============================================================================\n";
        cout << "                         SELECTED TEST CASE                                    \n";
        cout << "===============================================================================\n";

        cout << "\nProblem:     " << tests[i].problem << "\n";
        cout << "Constraints: " << tests[i].constraints << "\n";
    ...
    while (true)
    {
        ...
        if (ch == 1)
        {
            ...
            while (getline(input, line))
            {
                ...
                {
                    problem = line.substr(line.find(":") + 1);
                    while (!problem.empty() && problem[0] == ' ')
                        problem.erase(0, 1);
                }
                else if (line.find("Constraints:") != string::npos)
                {
                    constraints = line.substr(line.find(":") + 1);
                    while (!constraints.empty() && constraints[0] == ' ')
